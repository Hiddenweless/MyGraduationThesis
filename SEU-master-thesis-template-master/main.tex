...
\newif\ifblindreview
\blindreviewfalse  % 盲审版设为 true；正式版改为 false
\begin{document}

%% ----------------------------------------------------------------------------
%%                                 Meta Data
%% ----------------------------------------------------------------------------
\categorynumber{TP391.4} %《中国图书资料分类法》分类法
\UDC{004.9}              %《国际十进分类法UDC》的类号
\secretlevel{公开}        % 学位论文密级分为"公开"、"内部"、"秘密"和"机密"四种
\ifblindreview
    \studentid{\mbox{}}
\else
    \studentid{234804}      % 学号要完整，前面的零不能省略
\fi
...
%% ----------------------------------------------------------------------------
\title
    {\texorpdfstring{基于规划反馈的多智能体\\图推理方法研究}{基于规划反馈的多智能体图推理方法研究}}        % 论文中文标题
    {}         % 论文中文副标题，没有可以空着
    {Research on Planning-Feedback-Based Multi-Agent Graph Reasoning Methods}  % 论文英文标题
    {}            % 论文英文副标题，没有可以空着
...
%% ----------------------------------------------------------------------------
\author
    {\ifblindreview\mbox{}\else 王腾龙\fi}                        % 作者中文姓名
    {\ifblindreview\mbox{}\else WANG Teng-long\fi}                  % 作者英文姓名，首字母大写，姓名分开，双字用「-」连接

\advisor
    {林丽\ 讲师}                % 导师中文姓名及职称
    {LIN Li}        % 导师英文姓名
    {Assistant Professor}                     % 导师英文职称
\coadvisor
    {浦春林\ 高工}                % 校外导师中文姓名及职称
    {PU Chun-lin}        % 校外导师英文姓名
    {Senior Engineer}                     % 校外导师英文职称
%% ----------------------------------------------------------------------------
%%                              Thesis Defence
%% ----------------------------------------------------------------------------
...
\defenddate{2026 年 6 月 8 日}          % 答辩日期 \today
\authorizedate{}                  % 授予学位日期，这个档案袋不需要填
\ifblindreview
    \committeechair{\mbox{}}
    \reviewer{盲审}{盲审}
\else
    \committeechair{李传佑}               % 答辩委员会主席姓名
    \reviewer{盲审}{盲审}                % 两位论文评阅人姓名
\fi
\department                        % 学院名称
    {\ifblindreview\mbox{}\else 苏州联合研究生院\fi}
    {\ifblindreview\mbox{}\else Suzhou Joint Graduate School\fi}
% 固定封面日期，避免编译日期变化影响最终打印版本
\renewcommand{\todaychn}{2026 年 6 月 8 日}
\renewcommand{\todayeng}{June 8, 2026}
...
]{figures/defined/signed-declaration.pdf}
...
% \input{chapters/appendix}           % 附录（详细实验结果已并入正文第3、4章）
\ifblindreview
\else
    \input{chapters/resume}         % 作者成果页/作者简介
\fi
\ifblindreview
\else
