...
为保证 PlanGraph 系统的可复现性，同时避免在附录中堆叠过长的工程提示词文本，本文仅给出各智能体提示词和检索查询模板的摘要形式。完整模板在实验代码中固定保存。需要说明的是，QuestionAgent、PlanningAgent、CodingAgent 和 RepairAgent 采用提示词驱动；RetrievalAgent 不调用大语言模型，而是根据规划工件、任务约束和执行反馈构造检索查询。

\section{检索查询模板摘要}

RetrievalAgent 的作用是根据规划结果和反馈信息组织检索查询，而不是生成自然语言回答。其查询构造主要利用任务类型、图属性、算法提示、约束标签、输出格式以及执行错误信息。该设计使检索过程不直接依赖原始问题文本，而是围绕当前求解步骤和约束条件展开。
...
\paragraph{QuestionAgent。}
QuestionAgent 用于将原始自然语言问题解析为结构化图推理任务，输出字段包括 \texttt{task\_type}、\texttt{nodes}、\texttt{edges}、\texttt{directed}、\texttt{weighted}、\texttt{capacity}、\texttt{params}、\texttt{output\_format} 和 \texttt{constraints}。其核心模板如下：
...
\paragraph{PlanningAgent。}
PlanningAgent 用于生成算法级伪代码规划，并为检索阶段提供算法提示、约束标签和检索术语。其核心模板如下：

\begin{quote}
\small
\ttfamily
Produce structured algorithm-level pseudocode, not Python code.\\
Include graph construction, algorithm choice, constraint checks, and output formatting.\\
...
\paragraph{CodingAgent。}
CodingAgent 用于基于任务、规划和检索知识生成可执行 Python 程序。其核心模板如下：
...
\paragraph{RepairAgent。}
RepairAgent 用于根据执行反馈和检索反馈修复当前程序，主要处理图构建错误、API 使用错误、约束遗漏和输出格式不匹配等问题。其核心模板如下：
...
Revise API misuse, missing constraint handling, incorrect graph construction, or output mismatch.\\
...
上述模板摘要保留了 PlanGraph 在任务结构化、规划约束、检索查询构造和执行反馈修复方面的关键设计，同时避免完整工程提示词对附录篇幅造成过多占用。