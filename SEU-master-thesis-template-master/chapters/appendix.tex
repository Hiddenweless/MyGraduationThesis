...
为保证 PlanGraph 系统的可复现性，同时避免堆叠过长的工程提示词文本，本文仅给出各智能体提示词和检索查询模板的摘要形式。需要说明的是，问题智能体、规划智能体、编码智能体和修复智能体采用提示词驱动；检索智能体不调用大语言模型，而是根据规划工件、任务约束和执行反馈构造检索查询。
...
修复智能体用于根据执行反馈和检索反馈修复当前程序，主要处理图构建错误、API 使用错误、约束遗漏和输出格式不匹配等问题。其核心模板如下：
