...
为保证 PlanGraph 系统的可复现性，同时避免堆叠过长的工程提示词文本，本文仅给出各智能体提示词和检索查询模板的摘要形式。需要说明的是，QuestionAgent、PlanningAgent、CodingAgent 和 RepairAgent 采用提示词驱动；RetrievalAgent 不调用大语言模型，而是根据规划工件、任务约束和执行反馈构造检索查询。
